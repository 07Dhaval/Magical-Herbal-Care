  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Backend returned non-JSON. Check backend terminal.");
      }

      if (!res.ok || !data.success) {
        console.error("ADD PRODUCT BACKEND ERROR:", data);
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <input name="price" value={form.price} onChange={handleChange} placeholder="Price" type="text" className="input" />
          ...
          {[0, 1, 2].map((index) => (
            ...
            >
              ...
              {previewImages[index] ? (
                ...
                <>
                  <UploadCloud size={24} />
                  <p className="mt-2">Upload Image {index + 1}</p>
                </>
              )}
            </label>
          ))}
        ...
        <button
          type="submit"
          disabled={loading}
          className="bg-[#2f4f2f] text-white px-8 py-3 rounded-xl flex items-center gap-2 disabled:opacity-60"
        >
          <ImagePlus size={18} />
